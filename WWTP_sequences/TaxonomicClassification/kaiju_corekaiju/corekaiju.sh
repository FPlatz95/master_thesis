...
# run parallel
cat $output_dir1/wwtp_sequences.txt | parallel -j10 kaiju -t $db_refseq/nodes.dmp -f $db_core_pfam/corepfams_reduced.fmi -i $data_path/{}_R1.fastq -j $data_path/{}_R2.fastq -o $output_dir2/{}_corekaiju_tax.out -z $threads '&>' $output_dir1/log/{}_corekaiju.log

cat $output_dir1/wwtp_sequences.txt | parallel -j10 kaiju2table -t $db_refseq/nodes.dmp -n $db_refseq/names.dmp -r species -o $output_dir2/{}_corekaiju_summary.tsv $output_dir2/{}_corekaiju_tax.out -v -l superkingdom,phylum,class,order,family,genus,species '&>' $output_dir1/log/{}_corekaiju.log
