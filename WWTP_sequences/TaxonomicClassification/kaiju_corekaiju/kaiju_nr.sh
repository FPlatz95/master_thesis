...
# run parallel
cat $output_dir1/wwtp_sequences.txt | parallel -j5 kaiju -t $db_nr/nodes.dmp -f $db_nr/nr/kaiju_db_nr.fmi -i $data_path/{}_R1.fastq -j $data_path/{}_R2.fastq -o $output_dir2/{}_kaiju_nr_tax.out -z $threads '&>' $output_dir1/log/{}_kaiju_nr.log

cat $output_dir1/wwtp_sequences.txt | parallel -j5 kaiju2table -t $db_nr/nodes.dmp -n $db_nr/names.dmp -r species -o $output_dir2/{}_kaiju_nr_summary.tsv $output_dir2/{}_kaiju_nr_tax.out -v -l superkingdom,phylum,class,order,family,genus,species '&>' $output_dir1/log/{}_corekaiju.log
