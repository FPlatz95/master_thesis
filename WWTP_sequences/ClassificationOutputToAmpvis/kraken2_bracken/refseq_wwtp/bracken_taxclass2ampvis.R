...
kraken_convertotu=function(input){
  output=input%>%
    mutate(OTU=paste0("OTU",1:nrow(input),"_bracken"))%>%
    select(c(14,3,7:13)) %>%
    filter(V3 > 0) %>%
...
write.csv(bracken_otu, paste0("/srv/MA/Projects/microflora_danica/analysis/projects/MFD_seges/results/WWTP_sequences/kraken2/bac_refseq_WWTP/bracken_ampvis_otu/", filename, "_refseq_wwtp_bracken_ampvisotu.csv"), row.names = FALSE)
