...
bracken_load = read.delim(filepath, header = FALSE)
bracken_otu = kraken_convert(bracken_load)
bracken_otu = bracken_otu %>% rename(!!filename := V3)

bracken_otu$Kingdom = str_trim(bracken_otu$Kingdom)
bracken_otu$Phylum = str_trim(bracken_otu$Phylum)
bracken_otu$Class = str_trim(bracken_otu$Class)
bracken_otu$Order = str_trim(bracken_otu$Order)
bracken_otu$Family = str_trim(bracken_otu$Family)
bracken_otu$Genus = str_trim(bracken_otu$Genus)
bracken_otu$Species = str_trim(bracken_otu$Species)

bracken_otu$Kingdom = str_replace(bracken_otu$Kingdom, "d__", "")
bracken_otu$Phylum = str_replace(bracken_otu$Phylum, "p__", "")
bracken_otu$Class = str_replace(bracken_otu$Class, "c__", "")
bracken_otu$Order = str_replace(bracken_otu$Order, "o__", "")
bracken_otu$Family = str_replace(bracken_otu$Family, "f__", "")
bracken_otu$Genus = str_replace(bracken_otu$Genus, "g__", "")
bracken_otu$Species = str_replace(bracken_otu$Species, "s__", "")

write.csv(bracken_otu, paste0("/srv/MA/Projects/microflora_danica/analysis/projects/MFD_seges/results/WWTP_sequences/kraken2/GTDB/bracken_ampvis_otu/", filename, "_gtdb_bracken_ampvisotu.csv"), row.names = FALSE)
